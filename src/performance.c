...
uint32_t volatile _group_busy = 0;
uint32_t volatile _perf_busy = 0;
...
static inline void spin_lock(uint32_t volatile *busy);

static inline void spin_lock(uint32_t volatile *busy) {
  bool stopped = 0;
  while (qthread_cas32(busy, 0, 1) != 0) { stopped = 1; }
  if (stopped) { qtlog(LOGWARN, "Stopped in spinlock"); }
}
...
qtstategroup_t *qtperf_create_state_group(size_t num_states,
  ...
  if (_next_group == NULL) { _next_group = &_groups; }
...
void qtperf_free_state_group_internals(qtstategroup_t *group) {
  ...
  if (group->state_names != NULL) {
    size_t i = 0;
    for (i = 0; i < group->num_states; i++) { free(group->state_names[i]); }
...
void qtperf_free_perf_list(qtstategroup_t *group,
                           qtperf_perf_list_t *counters) {
...
void qtperf_start() {
  ...
  uint32_t volatile *last_busy = NULL;
  if (_collecting == 1) {
  ...
  for (data = qtperf_iter_next(&iter); data != NULL;
       data = qtperf_iter_next(&iter)) {
    if (last_busy != NULL) *last_busy = 0; // release previous
    last_busy = &data->busy;
    spin_lock(&data->busy);
...
char const *qtperf_state_name(qtstategroup_t *group, qtperfid_t state) {
  if (state >= group->num_states) {
    qtlogargs(
      LOGERR, "ERROR - STATE NUMBER %lu OUT OF BOUNDS FOR GROUP", state);
    return "ERROR";
...
void qtperf_enter_state(qtperfdata_t *data, qtperfid_t state) {
  ...
  if (state != QTPERF_INVALID_STATE &&
      state >= data->counters->state_group->num_states) {
  ...
  if (_collecting && from_state != QTPERF_INVALID_STATE) {
    if (data->time_entered < now / 2) {
      qtlogargs(LOGWARN,
                "Warning: entering state with invalid time_entered value %lu",
                data->time_entered);
    }
    spin_lock(&data->counters->busy);
  ...
  if (state == QTPERF_INVALID_STATE) {
    ...
    data->time_entered = now;
    if (data->piggybacks != NULL &&
        data->piggybacks[data->current_state] != NULL) {
...
qtperfdata_t *qtperf_iter_deref(qtperf_iterator_t *iter) {
  if (iter == qtperf_iter_end()) { return NULL; }
...
qtperf_iterator_t *qtperf_iter_end() { return NULL; }

/* This section is for the instrumentation of workers */

static char const *worker_state_names[] = {
  "WKR_INIT", "WKR_QTHREAD_ACTIVE", "WKR_SHEPHERD", "WKR_IDLE", "WKR_BLOCKED"};
...
void qtperf_set_instrument_workers(bool yes_no) {
  ...
  if (yes_no && qtperf_workers_group == NULL) {
    qtperf_workers_group = qtperf_create_state_group(
      WKR_NUM_STATES, "Workers Internal", worker_state_names);
  }
  if (qtperf_should_instrument_workers) {
    QTPERF_ASSERT(qtperf_should_instrument_workers &&
                  (qtperf_workers_group != NULL));
  }
}

/* This section is for instrumentation of qthreads */
static char const *qthread_state_names[] = {
...
void qtperf_set_instrument_qthreads(bool yes_no) {
  QTPERF_ASSERT(
    QTHREAD_STATE_NUM_STATES == 16 &&
    "threadstate_t has changed, check to make sure all states are represented "
    "in qthread_state_names in performance.c"); // make sure we're still current
                                                // with our names array.
  qtperf_should_instrument_qthreads = yes_no;

  if (yes_no && qtperf_qthreads_group == NULL) {
    qtperf_qthreads_group = qtperf_create_state_group(
      QTHREAD_STATE_NUM_STATES, "Qthreads Internal", qthread_state_names);
  }
  if (qtperf_should_instrument_qthreads) {
    QTPERF_ASSERT(qtperf_should_instrument_qthreads &&
                  (qtperf_qthreads_group != NULL));
  }
}
...
void qtperf_piggyback_state(qtperfdata_t *source_data,
  ...
  if (source_data->piggybacks == NULL) {
    source_data->piggybacks =
      calloc(source_data->counters->num_states, sizeof(qtperfdata_t *));
    memset(source_data->piggybacks,
           0,
           sizeof(qtperfdata_t *) * source_data->counters->num_states);
  }
...
qtperfdata_t *qtperf_get_qthread_data(void) {
  ...
  if (me->rdata->performance_data == NULL) {
    qtlog(LOGWARN,
          "qtperf_get_qthread_data() called, but "
          "qtperf_set_instrument_qthreads() has not been called");
    return NULL;
  }
 ...
 * database:
 *
 * myprog | egrep '^-' | q -d, 'select c2,1.0*c1/c2 as ratio from - order by
 * ratio'
 */
...
void qtperf_print_delimited(qtstategroup_t *group,
  ...
  size_t i = 0;
  char const *pfx = "";
  if (row_prefix != NULL) pfx = row_prefix;
  ...
  for (current = group->counters, i = 0; current != NULL;
...
void qtperf_print_group(qtstategroup_t *group) {
  ...
  printf("%s (%lu instances, %lu total states, total time=%llu)\n",
         ...
         group->num_counters,
         group->num_states,
         qtperf_total_group_time(group));
  for (current = group->counters, i = 0; current != NULL;
       current = current->next, i++) {
    printf("  Instance %lu, total time %llu:\n",
           i,
           qtperf_total_time(&current->performance_data));
    qtperf_print_perfdata(&current->performance_data,
                          PERF_SHOW_STATES_WITH_ZERO_TIME);
  }
  printf("------------------------------------------------\n");
}
...
void qtperf_print_perfdata(qtperfdata_t *perfdata, bool show_zeros) {
  size_t i = 0;
  char const **names =
    (char const **)perfdata->counters->state_group->state_names;
  qtperfcounter_t *data = perfdata->counters->data;
  if (!perfdata
         ->ctr_owner) { // don't print individual records for aggregated groups
    return;
  }
...
bool qtp_validate_names(char const **names, size_t count) {
  ...
  for (i = 0; names != NULL && i < count; i++) {
    ...
    valid = valid && printable && len < MAX_NAME_LENGTH && len > 0 &&
            names[i][len] == '\0';
    assert_true(printable && len < MAX_NAME_LENGTH && len > 0 &&
                names[i][len] == '\0');
  }
...
bool qtp_validate_piggyback_list(qtperf_piggyback_list_t *list) {
  ...
  while (current != NULL) {
    ...
    assert_true(valid = valid && (current->target_state <
                                  current->target_data->counters->num_states));
...
bool qtp_validate_piggybacks(qtperf_piggyback_list_t **piggybacks,
                             size_t num_states) {
  size_t counter = 0;
  bool valid = 1;
  if (piggybacks == NULL) { return valid; }
...
bool qtp_validate_perfdata(qtperfdata_t *data) {
  ...
  qtlogargs(PERFDBG,
            "data->current_state=%lu, data->time_entered=%lu",
            data->current_state,
            data->time_entered);
  assert_true(
...
bool qtp_validate_state_group(qtstategroup_t *group) {
  ...
  valid_names =
    qtp_validate_names((char const **)group->state_names, group->num_states);
  valid_counters = qtp_validate_perf_list(
    group->counters, group->next_counter, group->num_counters);
