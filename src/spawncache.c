...
qt_threadqueue_private_t INTERNAL *qt_init_local_spawncache(void) {
  qt_threadqueue_private_t *const ret = qt_internal_aligned_alloc(
    sizeof(qt_threadqueue_private_t), qthread_cacheline());
...
void INTERNAL qt_spawncache_filter(qt_threadqueue_filter_f f) {
  ...
  if (cache) { qt_threadqueue_private_filter(cache, f); }
}
...
int INTERNAL qt_spawncache_flush(qt_threadqueue_t *q) {
  ...
  if (cache) {
    qthread_debug(
      THREADQUEUE_BEHAVIOR, "attempting to flush cache %p\n", cache);
    if (cache->on_deck) {
      qthread_debug(THREADQUEUE_DETAILS,
                    "attempting to flush cache %p into queue %p : %u items\n",
                    cache,
                    q,
                    cache->qlength + 1);
