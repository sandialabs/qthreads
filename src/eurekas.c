...
static void eureka(void) { /*{{{*/
  ...
  /* 7: exit barrier */
  {
    ...
    if (t) {
      if (t->thread_state == QTHREAD_STATE_ASSASSINATED) {
        if (t->rdata->criticalsect > 0) { barrier_participation = 0; }
      }
    }
  ...
  if (t) {
    if (t->thread_state == QTHREAD_STATE_ASSASSINATED) {
      tassert(t->rdata);
      if (t->rdata->criticalsect == 0) { qthread_back_to_master2(t); }
    }
  }
} /*}}}*/
...
void INTERNAL qt_eureka_worker_init(void) { /*{{{*/
  ...
  /* Yes, these could have default values, but because of lazy TLS allocation,
   * these MUST be set manually */
  TLS_INIT(eureka_block);
  TLS_INIT(eureka_blocked_flag);
  TLS_SET(eureka_block, 0);
  TLS_SET(eureka_blocked_flag, 0);
#if 0 // This is commented out because sigaltstack() does not appear to work
      // with pthreads reliably
...
static filter_code qt_eureka_internal_filterfunc(qt_key_t const addr,
                                                 qthread_t *restrict const t,
                                                 void *restrict arg) { /*{{{*/
  ...
  if (t->ret) {
    if (t->flags & QTHREAD_RET_IS_SYNCVAR) {
      ...
      qthread_debug(FEB_DETAILS,
                    "tid %u assassinated, filling retval (%p)\n",
                    t->thread_id,
                    t->ret);
...
qt_team_critical_section(qt_team_critical_section_t boundary) { /*{{{*/
  ...
  assert(self);
  switch (boundary) {
    case BEGIN: self->rdata->criticalsect++; break;
    case END:
...
void API_FUNC qt_team_eureka(void) { /*{{{*/
  ...
  qthread_debug(TEAM_DETAILS,
                "setting my team's (%u) eureka batsignal (%p) to %u\n",
                my_team->team_id,
                &my_team->eureka,
                TEAM_SIGNAL_EUREKA(my_team->team_id));
  ...
  for (qthread_shepherd_id_t shep = 0; shep < qlib->nshepherds; shep++) {
    ...
    for (unsigned int wkrid = 0; wkrid < qlib->nworkerspershep; wkrid++) {
      int ret;
      if (wkr == &wkrs[wkrid]) { continue; }
      signalcount++;
      if ((ret = pthread_kill(wkrs[wkrid].worker, QT_EUREKA_SIGNAL)) != 0) {
        qthread_debug(
          ALWAYS_OUTPUT, "pthread_kill failed: %s\n", strerror(ret));
        QTHREAD_TRAP(); // cannot be abort, because we're called from a signal
                        // handler, and abort makes things go WEIRD
      }
    }
  }
  ...
  if ((my_team->parent_id != QTHREAD_DEFAULT_TEAM_ID) &&
    ...
    qt_sinc_reset(my_team->sinc,
                  2); // I, and the waiter, are the only remaining members
  } else {
    qt_sinc_reset(my_team->sinc, 1); // I am the only remaining member
  }
  qt_sinc_submit(my_team->subteams_sinc,
                 NULL); // wait for subteams to die (if any)
  qthread_debug(
    TEAM_DETAILS, "wait for subteams to die... %p\n", &my_team->subteams_sinc);
  ...
  assert(self->ret == NULL ||
         self->ret ==
           my_team->return_loc); // XXX: what should we do if this is not true?
...
void INTERNAL qt_eureka_check(int block) { /*{{{*/
  ...
  clear_flag:
    ...
    if (block) { TLS_SET(eureka_block, 1); }
  } else if (!block) {
    TLS_SET(eureka_block, 0);
  }
  if (TLS_GET(eureka_blocked_flag)) { goto clear_flag; }
} /*}}}*/
