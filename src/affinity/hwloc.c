...
DEBUG_ONLY(static char const *typename);
  ...
  if (*nbshepherds == 0) { /* we need to guesstimate */
    ...
    if (shep_type_idx == -1) {
      do {
        ...
        qthread_debug(
          AFFINITY_DETAILS,
          "num objs of type %i = %d\n",
          shep_type_idx,
          hwloc_get_nbobjs_inside_cpuset_by_type(
            topology, allowed_cpuset, shep_type_options[shep_type_idx]));
        if ((shep_type_idx == 0) &&
            (hwloc_get_nbobjs_inside_cpuset_by_type(
      ...
      if ((shep_type_idx > 0) &&
        ...
        if (cacdepth == maxdepth) {
          qthread_debug(AFFINITY_DETAILS,
                        ...
                        (unsigned int)hwloc_get_nbobjs_inside_cpuset_by_type(
                          topology, allowed_cpuset, HWLOC_OBJ_PU));
    ...
    *nbworkers = guess_num_workers_per_shep(*nbshepherds);
    ...
    if (shep_type_idx != -1) {
      ...
      if (shep_depth <= 0) {
        qthread_debug(AFFINITY_DETAILS,
                      "invalid shepherd type (%s), finding another one...\n",
                      typenames[shep_type_idx]);
    ...
    if (shep_type_idx == -1) {
      ...
      for (realdepth = 0; realdepth < maxdepth && shep_depth == -1;
        ...
        qthread_debug(
          AFFINITY_DETAILS,
          "%s at depth %u (nbobjs is %u%s)\n",
          hwloc_obj_type_string(hwloc_get_depth_type(topology, realdepth)),
          realdepth,
          num,
          (num == HWLOC_TYPE_DEPTH_UNKNOWN)
            ? " (unknown)"
            : ((num == HWLOC_TYPE_DEPTH_MULTIPLE) ? " (multiple)" : ""));
        if (num == *nbshepherds) {
          shep_depth = realdepth;
          DEBUG_ONLY(typename = hwloc_obj_type_string(
      ...
      if ((shep_depth == -1) && (fl_depth != -1)) {
        ...
        shep_depth = fl_depth;
        DEBUG_ONLY(typename = hwloc_obj_type_string(
                     hwloc_get_depth_type(topology, fl_depth)));
        qthread_debug(AFFINITY_DETAILS,
                      "first larger depth is %i, %s\n",
                      fl_depth,
                      typename);
      }
    ...
    qthread_debug(AFFINITY_CALLS,
                  "Using depth %d (%s) for %d shepherds\n",
                  shep_depth,
                  typename,
                  *nbshepherds);
...
qthread_shepherd_id_t INTERNAL guess_num_shepherds(void) { /*{{{ */
  ...
  DEBUG_ONLY(hwloc_topology_check(topology));
  DEBUG_ONLY(hwloc_const_cpuset_t allowed_cpuset =
               hwloc_topology_get_allowed_cpuset(topology);)
  ret = num_usable_by_depth(shep_depth);
  qthread_debug(
    AFFINITY_BEHAVIOR,
    "of %u %s's, only %u %s viable\n",
  ...
  qthread_debug(
    AFFINITY_FUNCTIONS,
    "guessing %u shepherds (nbobjs_by_depth, depth: %u=%s)\n",
    (unsigned int)ret,
    shep_depth,
    hwloc_obj_type_string(hwloc_get_depth_type(topology, shep_depth)));
  if (ret == 0) { ret = 1; }
...
guess_num_workers_per_shep(qthread_shepherd_id_t nshepherds) { /*{{{ */
  ...
  if (wkr_index != -1) {
    qthread_debug(AFFINITY_BEHAVIOR,
                  "guessing number of workers based on worker type\n");
    assert(shep_depth >= 0);
    ...
    if (hwloc_compare_types(wkr_type, HWLOC_OBJ_CACHE_UNIFIED) == 0) {
      qthread_debug(AFFINITY_DETAILS,
                    "worker type is a cache; must handle specially...\n");
      ...
      for (int wkridx = 5; wkridx <= wkr_index; wkridx++) {
        hwloc_obj_t tmp;
        qthread_debug(
          AFFINITY_DETAILS, "wkridx=%i wkr_index=%i\n", wkridx, wkr_index);
        do {
          wkr_depth--;
          ...
          qthread_debug(
            AFFINITY_DETAILS,
            "wkr_depth %i, type = %s %i\n",
            wkr_depth,
            hwloc_obj_type_string(tmp->type),
            hwloc_compare_types(tmp->type, HWLOC_OBJ_CACHE_UNIFIED));
  ...
  guess_my_weight:
    wkr_depth = hwloc_get_type_depth(topology, HWLOC_OBJ_PU);
    qthread_debug(AFFINITY_BEHAVIOR,
                  "guessing number of workers based on shepherd \"WEIGHT\", "
                  "wkr_depth = %u\n",
                  wkr_depth);
  ...
  qthread_debug(AFFINITY_FUNCTIONS,
                "guessed %i workers apiece for %i sheps\n",
                (int)ret,
                (int)nshepherds);
  return ret;
} /*}}} */
...
void INTERNAL qt_affinity_set(qthread_worker_t *me,
  ...
  qthread_debug(AFFINITY_DETAILS,
                "num_wkrs = %u\n",
                hwloc_get_nbobjs_inside_cpuset_by_depth(
                  topology, allowed_cpuset, wkr_depth));
  qthread_debug(
    AFFINITY_DETAILS,
    "num_wkrs = %u\n",
...
#ifdef QTHREAD_DEBUG_AFFINITY
  {
    ...
    qthread_debug(
      ...
      (int)me->worker_id,
      (int)me->packed_worker_id,
      (shep_pus * myshep->node) +
        ((me->worker_id + (wraparounds * nworkerspershep)) % shep_pus),
      (shep_pus * myshep->node) +
        (((me->worker_id * worker_pus) + (wraparounds * nworkerspershep) +
          (worker_wraparounds)) %
         shep_pus),
...
int INTERNAL qt_affinity_gendists(qthread_shepherd_t *sheps,
  ...
  num_extant_objs = hwloc_get_nbobjs_inside_cpuset_by_depth(
    topology, allowed_cpuset, shep_depth);
  qthread_debug(AFFINITY_DETAILS,
                "found %u objects at shep_depth %u\n",
                num_extant_objs,
                shep_depth);
  ...
  if (matrix) {
    qthread_debug(AFFINITY_DETAILS,
                  ...
                  hwloc_obj_type_string(
                    hwloc_get_depth_type(topology, matrix->relative_depth)),
                  matrix->nbobjs);
