...
static unsigned long volatile *timer_address = NULL;
...
void qtimer_start(qtimer_t q) { q->start = *timer_address; }

unsigned long qtimer_fastrand(void) {
  if (NULL == timer_address) {
    if (0 != qtimer_init()) { return 0; }
  }
...
double qtimer_res(void) { return timer_freq_conv; }

void qtimer_stop(qtimer_t q) { q->stop = *timer_address; }
