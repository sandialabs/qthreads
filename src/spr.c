...
static qthread_f spr_remote_functions[2] = {spr_locale_barrier_op, NULL};

// Locale barrier
static qt_barrier_t *locale_barrier = NULL;

static void call_fini(void) { spr_fini(); }
...
int spr_init(unsigned int flags, qthread_f *regs) {
  ...
  if (regs) { spr_register_actions(regs, 0, spr_init_base); }
...
int spr_fini(void) {
  ...
  if (NULL != locale_barrier) { qt_barrier_destroy(locale_barrier); }

  if (initialized_flags & SPR_SPMD) { qthread_multinode_multistop(); }
...
int spr_register_actions(qthread_f *actions, size_t count, size_t base) {
  ...
  if (count == 0) {
    ...
    for (int i = 0; i < count; i++) {
      qassert(qthread_multinode_register(i + base, actions[i]),
              QTHREAD_SUCCESS);
    }
  }
...
int spr_get_nb(void *restrict dest_addr,
  ...
  qthread_empty(&h->feb);
  rc = qthread_internal_net_driver_get(
    dest_addr, src_loc, src_addr, size, &h->feb);

  qthread_debug(MULTINODE_CALLS,
                "[%d] end spr_get_nb(%d, %p, %p, %d)\n",
                spr_locale_id(),
                src_loc,
                src_addr,
                dest_addr,
...
int spr_put_nb(int dest_loc,
  ...
  qthread_empty(&h->feb);
  rc = qthread_internal_net_driver_put(
    dest_loc, dest_addr, src_addr, size, &h->feb);

  qthread_debug(MULTINODE_CALLS,
                "[%d] end spr_put_nb(%d, %p, %p, %d)\n",
                spr_locale_id(),
                dest_loc,
                dest_addr,
                src_addr,
...
aligned_t spr_locale_barrier_op(void *arg_) {
  ...
  qthread_debug(
    MULTINODE_CALLS, "[%d] enter task-level barrier.\n", spr_locale_id());

  assert(NULL != locale_barrier); // Must run on L0
  qt_barrier_enter(locale_barrier);

  qthread_debug(
    MULTINODE_CALLS, "[%d] exit task-level barrier.\n", spr_locale_id());
...
int spr_locale_barrier(void) {
  ...
  if (0 == spr_initialized) {
    ...
    qthread_debug(MULTINODE_BEHAVIOR,
                  "[%d] enter network driver runtime  barrier.\n",
                  spr_locale_id());

    rc = qthread_internal_net_driver_barrier();

    qthread_debug(MULTINODE_BEHAVIOR,
                  "[%d] exit network driver runtime  barrier.\n",
                  spr_locale_id());
  } else {
    // Perform task-level barrier
    qthread_debug(
      MULTINODE_BEHAVIOR, "[%d] enter task-level barrier.\n", spr_locale_id());
    ...
    qthread_debug(
      MULTINODE_BEHAVIOR, "[%d] exit task-level barrier.\n", spr_locale_id());
  }
