...
#ifdef ATOMIC_ALIGNED_AVX
  // Zero out the loaded work object in debug mode.
#ifndef NDEBUG
  __uint128_t work_tmp = atomic_exchange_explicit(
    (_Atomic uint128_t *)&control->work, uint128_t(0u), memory_order_relaxed);
#else
  __uint128_t work_tmp = atomic_load_explicit(
    (_Atomic __uint128_t *)&control->work, memory_order_relaxed);
#endif
  pooled_thread_work work = *(pooled_thread_work *)&work_tmp;
  return work;
#else
...
#error "not implemented correctly yet."
  ...
  struct {
    ...
    _Atomic uint32_t;
    pool_header *pool;
  } control_slice = {0u, i, &hw_pool};

  atomic_store_explicit(
    (_Atomic __uint128_t *)&thread_control->work, 0u, memory_order_relaxed);
  atomic_store_explicit((_Atomic __uint128_t *)&thread_control->waiting_flag,
                        *(__uint128_t *)&control_slice,
...
API_FUNC pool_init_status pool_init(uint32_t num_threads) {
...
API_FUNC __attribute__((no_sanitize("memory"))) void pool_destroy() {
...
API_FUNC void pool_run_on_all(qt_threadpool_func_type func, void *arg) {
  uint32_t num_threads =
    atomic_load_explicit(&hw_pool.num_threads, memory_order_relaxed);
  assert(num_threads);
  assert(num_threads < UINT32_MAX);
  char *buffer =
    (char *)atomic_load_explicit(&hw_pool.threads, memory_order_relaxed);
  atomic_store_explicit(
    &hw_pool.num_active_threads, num_threads, memory_order_relaxed);
  init_main_sync(&hw_pool);
  size_t alignment = QTHREAD_MAX((size_t)64u, get_cache_line_size());
  for (uint32_t i = 0u;
       i < atomic_load_explicit(&hw_pool.num_threads, memory_order_relaxed);
       i++) {
    pooled_thread_control *thread_control =
      (pooled_thread_control *)(buffer + alignment * (size_t)i);
    launch_work_on_thread(thread_control, func, arg);
  }
  suspend_main_while_working(&hw_pool);
}
