...
static void qt_hash_internal_create(qt_hash ret, size_t entries)
{
    ...
    /* data storage */
    posix_memalign((void**)&(ret->entries), linesize, sizeof(hash_entry) * entries);
    if (ret->entries) {
...
static inline void** qt_hash_internal_find(qt_hash h, qt_key_t key)
{
	...
	if (z[bucket+i].key == key) return (void*)&z[bucket+i].value;
	if (z[bucket+i].key == KEY_NULL) return NULL;
    }
...
static void brehash(qt_hash h, size_t len)
{
    ...
    free(h->entries);
    memcpy(h, d, sizeof(struct qt_hash_s));
    free(d);
}
