...
#define RLIMIT_TO_NORMAL(thr)                                                  \
  do {                                                                         \
    qthread_debug(THREAD_DETAILS,                                              \
                  ...
                  (thr),                                                       \
                  (thr)->thread_id);                                           \
    if (!(atomic_load_explicit(&(thr)->flags__, memory_order_relaxed) &        \
          QTHREAD_REAL_MCCOY)) {                                               \
      struct rlimit rlp;                                                       \
...
static void *qthread_master(void *arg) {
  ...
  while (!done) {
    ...
    if (atomic_load_explicit(&t->thread_state, memory_order_relaxed) ==
      ...
      assert(t->f != NULL ||
             atoic_load_explicit(&t->flags__, memory_order_relaxed) &
               QTHREAD_REAL_MCCOY);
      if (t->rdata == NULL) {
        alloc_rdata(me, t);
      } else {
...
void API_FUNC qthread_finalize(void) { /*{{{ */
  ...
  if ((qlib == NULL) || (qthread_shep() != 0) || (qthread_worker(NULL) != 0)) {
    ...
    if (0 == (atomic_load_explicit(&t->flags__, memory_order_relaxed) &
              QTHREAD_REAL_MCCOY)) {
      return;
    }
  }
  ...
  for (i = 0; i < qlib->nshepherds; i++) {
    ...
    for (j = 0; j < qlib->nworkerspershep; j++) {
      ...
      atomic_store_explicit(
        &t->flags__, QTHREAD_UNSTEALABLE, memory_order_relaxed);
...
void API_FUNC *qthread_get_tasklocal(unsigned int size) { /*{{{*/
  ...
  if (NULL != f) {
    ...
    if ((0 == tl_sz) && (size <= qlib->qthread_tasklocal_size)) {
      // Use default space
      if (atomic_load_explicit(&f->flags__, memory_order_relaxed) &
          QTHREAD_BIG_STRUCT) {
...
static QINLINE qthread_t *qthread_thread_new(qthread_f const f,
  ...
  if (arg_size > 0) {
    if (arg_size <= qlib->qthread_argcopy_size) {
      t->arg = (void *)(&t->data);
      atomic_store_explicit(
        &t->flags__, QTHREAD_BIG_STRUCT, memory_order_relaxed);
    } else {
      t->arg = MALLOC(arg_size);
      atomic_store_explicit(
        &t->flags__, QTHREAD_HAS_ARGCOPY, memory_order_relaxed);
    }
  ...
  if (team_leader) {
    atomic_fetch_or_explicit(
      &t->flags__, QTHREAD_TEAM_LEADER, memory_order_relaxed);
  }
  ...
  if (atomic_load_explicit(&t->flags__, memory_order_relaxed) &
    ...
    if (NULL != t->team) {
      qt_internal_teamfinish(
        t->team, atomic_load_explicit(&t->flags__, memory_order_relaxed));
    }
    ...
    } else if (atomic_load_explicit(&t->flags__, memory_order_relaxed) &
    ...
    switch (k) {
...
#ifdef QTHREAD_USE_SPAWNCACHE
      {
        ...
        if (pq->on_deck) {
          ...
          assert(nt);
          if (((atomic_load_explicit(&nt->flags__, memory_order_relaxed) &
                QTHREAD_SIMPLE) != 0) ||
...
int API_FUNC qthread_spawn(qthread_f f,
  ...
  if (ret) {
    ...
    switch (ret_type) {
      case QTHREAD_SPAWN_RET_SYNCVAR_T:
        atomic_fetch_or_explicit(
          &t->flags__, QTHREAD_RET_IS_SYNCVAR, memory_order_relaxed);
      ...
      case QTHREAD_SPAWN_RET_SINC:
        atomic_fetch_or_explicit(
          &t->flags__, QTHREAD_RET_IS_SINC, memory_order_relaxed);
        break;
      case QTHREAD_SPAWN_RET_SINC_VOID:
        atomic_fetch_or_explicit(
          &t->flags__, QTHREAD_RET_IS_VOID_SINC, memory_order_relaxed);
        break;
      default:
  ...
  if (feature_flag & QTHREAD_SPAWN_NETWORK)
    atomic_fetch_or_explicit(
      &t->flags__, QTHREAD_NETWORK, memory_order_relaxed);
...
void INTERNAL qthread_back_to_master(qthread_t *t) { /*{{{ */
  assert((atomic_load_explicit(&t->flags__, memory_order_relaxed) &
          QTHREAD_SIMPLE) == 0);
  RLIMIT_TO_NORMAL(t);
#ifdef QTHREAD_PERFORMANCE
  QTPERF_WORKER_ENTER_STATE(qthread_internal_getworker()->performance_data,
...
int API_FUNC qthread_migrate_to(qthread_shepherd_id_t const shepherd) { /*{{{ */
  ...
  if (atomic_load_explicit(&me->flags__, memory_order_relaxed) &
      QTHREAD_REAL_MCCOY) {
  ...
  if (me->rdata->shepherd_ptr->shepherd_id == shepherd) {
    ...
    atomic_fetch_or_explicit(
      &me->flags__, QTHREAD_UNSTEALABLE, memory_order_relaxed);
    return QTHREAD_SUCCESS;
  }
  if (shepherd == NO_SHEPHERD) {
    me->target_shepherd = NO_SHEPHERD;
    atomic_fetch_and_explicit(
      &me->flags__, ~(uint8_t)QTHREAD_UNSTEALABLE, memory_order_relaxed);
    return QTHREAD_SUCCESS;
  }
