...
static void net_cleanup(void) {
  ...
  if (my_rank == 0) {
    ...
    for (i = 1; i < world_size; ++i) {
      ...
      msg.my_rank = my_rank;
      qthread_debug(
        MULTINODE_DETAILS, "[%d] sending die message to %d\n", my_rank, i);
...
static aligned_t fork_helper(void *info) {
  ...
  if (NULL != f) {
    ...
    if (0 != msg->return_addr) {
      ...
      qthread_debug(MULTINODE_DETAILS,
                    "[%d] sending return msg 0x%lx, %ld\n",
                    my_rank,
                    ret_msg.return_addr,
                    ret_msg.return_val);
      qthread_internal_net_driver_send(
        msg->origin_node, RETURN_MSG_TAG, &ret_msg, sizeof(ret_msg));
    }
  } else {
...
static void fork_msg_handler(int tag, void *start, size_t len) {
...
static void fork_long_msg_handler(int tag, void *start, size_t len) {
  qthread_debug(
    MULTINODE_FUNCTIONS, "[%d] begin fork_long_msg_handler\n", my_rank);

  qthread_fork_copyargs(fork_long_helper, start, len, NULL);

  qthread_debug(
    MULTINODE_FUNCTIONS, "[%d] end fork_long_msg_handler\n", my_rank);
}

static void return_msg_handler(int tag, void *start, size_t len) {
  struct return_msg_t *msg = (struct return_msg_t *)start;

  qthread_debug(MULTINODE_FUNCTIONS,
                "[%d] begin return_msg_handler 0x%lx, %ld\n",
                my_rank,
                (unsigned long)msg->return_addr,
                msg->return_val);
...
static void return_long_msg_handler(int tag, void *start, size_t len) {
  struct return_msg_t *msg = (struct return_msg_t *)start;

  qthread_debug(MULTINODE_FUNCTIONS,
                "[%d] begin return_long_msg_handler 0x%lx, %ld\n",
                my_rank,
                (unsigned long)msg->return_addr,
                msg->return_val);
  ...
  // TODO: figure out how to free long message. This is only called when there
  // is
  //       a return value.

  qthread_debug(
    MULTINODE_FUNCTIONS, "[%d] end return_long_msg_handler\n", my_rank);
}

static void die_msg_handler(int tag, void *start, size_t len) {
...
int qthread_multinode_initialize(void) {
  ...
  qthread_internal_net_driver_register(RETURN_LONG_MSG_TAG,
                                       return_long_msg_handler);
  ...
  if (0 != ret) {
    qthread_debug(MULTINODE_FUNCTIONS,
                  "qthread_internal_net_driver_init failed: %d\n",
                  ret);
    return ret;
  }
  ...
  if (0 != my_rank) { qthread_empty(&time_to_die); }

  /* make sure we can clean up */
  qthread_internal_cleanup_early(net_cleanup);

  qthread_debug(
    MULTINODE_CALLS, "[%d] end qthread_multinode_initialize\n", my_rank);
...
int qthread_multinode_multistart(void) {
  ...
  qthread_debug(
    MULTINODE_CALLS, "[%d] begin qthread_multinode_multistart\n", my_rank);

  qthread_internal_net_driver_barrier();

  qthread_debug(
    MULTINODE_CALLS, "[%d] end qthread_multinode_multistart\n", my_rank);
...
int qthread_multinode_multistop(void) {
  ...
  qthread_debug(
    MULTINODE_CALLS, "[%d] begin qthread_multinode_multistop\n", my_rank);
  ...
  qthread_debug(
    MULTINODE_CALLS, "[%d] end qthread_multinode_multistop\n", my_rank);

  return QTHREAD_SUCCESS;
}

int qthread_multinode_rank(void) { return my_rank; }

int qthread_multinode_size(void) { return world_size; }
...
int qthread_multinode_register(uint32_t uid, qthread_f f) {
  ...
  qthread_debug(MULTINODE_CALLS,
                "[%d] begin qthread_multinode_register(uid=%d, ptr=0x%lx)\n",
                my_rank,
                uid,
                (unsigned long)f);
  ...
  if (ret != 1) {
    qthread_debug(
      MULTINODE_DETAILS, "[%d] uid -> ptr registration failed\n", my_rank);
    return 1;
  }
  ...
  if (NULL != qt_hash_get(ptr_to_uid_hash, f)) {
    fprintf(stderr,
            "duplicate registration of action function 0x%lx\n",
            (unsigned long)f);
    return 1;
  }
  ...
  if (ret != 1) {
    qthread_debug(
      MULTINODE_DETAILS, "[%d] ptr -> uid registration failed\n", my_rank);
    return 1;
  }

  qthread_debug(
    MULTINODE_CALLS, "[%d] end qthread_multinode_register\n", my_rank);

  return 0;
}

int qthread_multinode_action_uid(qthread_f f, uint32_t *uid) {
  int ret = 0;
...
int qthread_multinode_action_ptr(uint32_t uid, qthread_f *f) {
  int ret = 0;
...
int qthread_fork_remote(
  ...
  qthread_debug(
    MULTINODE_CALLS,
    "[%d] begin f=0x%lx, arg=0x%lx, ret=0x%lx, rank=%d, arg_len=%ld)\n",
    my_rank,
    (unsigned long)f,
    (unsigned long)arg,
    (unsigned long)ret,
  ...
  if (arg_len <= FORK_MSG_PAYLOAD) {
    ...
    qthread_debug(MULTINODE_DETAILS,
                  "[%d] remote fork %d %d 0x%lx %d\n",
                  my_rank,
                  rank,
                  msg.uid,
                  msg.return_addr,
                  msg.arg_len);
    return qthread_internal_net_driver_send(
      rank, SHORT_MSG_TAG, &msg, sizeof(msg));
  } else {
    ...
    qthread_debug(
      ...
      rank,
      long_msg->uid,
      long_msg->return_addr,
      long_msg->arg_len);
    int const rc = qthread_internal_net_driver_send(
      rank, LONG_MSG_TAG, long_msg, long_msg_size);
