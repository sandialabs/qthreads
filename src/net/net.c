...
static volatile aligned_t num_ended = 0;
...
static void
fork_msg_handler(int tag, void *start, size_t len)
{
...
static void
return_msg_handler(int tag, void *start, size_t len)
{
...
static void
die_msg_handler(int tag, void *start, size_t len)
{
...
int
qthread_multinode_initialize(void)
{
    int ret;
...
int
qthread_multinode_register(uint32_t uid, qthread_f f)
{
    int ret = 0;
...
int
qthread_fork_remote(qthread_f f,
                    const void *arg,
                    aligned_t *ret,
                    int rank,
                    size_t arg_len)
