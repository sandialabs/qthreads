...
static int
encode(void const *inval, int invallen, char *outval, int outvallen) {
  static unsigned char encodings[] = {'0',
                                      '1',
                                      '2',
                                      '3',
                                      '4',
  ...
  if (invallen * 2 + 1 > outvallen) { return 1; }
...
static int decode(char const *inval, void *outval, int outvallen) {
  int i;
  char *ret = (char *)outval;

  if (outvallen != strlen(inval) / 2) { return 1; }
...
int qthread_internal_net_driver_runtime_init(void) {
  ...
  if (PMI_SUCCESS != PMI_Initialized(&initialized)) { return 1; }

  if (!initialized) {
    if (PMI_SUCCESS != PMI_Init(&initialized)) { return 2; }
  }

  if (PMI_SUCCESS != PMI_Get_rank(&rank)) { return 3; }

  if (PMI_SUCCESS != PMI_Get_size(&size)) { return 4; }
...
qthread_internal_net_driver_runtime_get_mapping(ptl_handle_ni_t ni_h) {
  ...
  for (i = 0; i < 4; ++i) {
    if (maps[i].handle == ni_h) { return maps[i].mapping; }
  }
  ...
  if (PMI_SUCCESS != PMI_KVS_Get_key_length_max(&max_key_len)) { return NULL; }
  ...
  if (PMI_SUCCESS != PMI_KVS_Get_my_name(name, max_name_len)) { return NULL; }
  ...
  snprintf(key,
           ...
           (long unsigned)ni_h,
           (long unsigned)rank);
  if (0 != encode(&my_id.phys.nid, sizeof(my_id.phys.nid), val, max_val_len)) {
    return NULL;
  }
  if (PMI_SUCCESS != PMI_KVS_Put(name, key, val)) { return NULL; }
  ...
  snprintf(key,
           ...
           (long unsigned)ni_h,
           (long unsigned)rank);
  if (0 != encode(&my_id.phys.pid, sizeof(my_id.phys.pid), val, max_val_len)) {
    return NULL;
  }
  if (PMI_SUCCESS != PMI_KVS_Put(name, key, val)) { return NULL; }

  if (PMI_SUCCESS != PMI_KVS_Commit(name)) { return NULL; }

  if (PMI_SUCCESS != PMI_Barrier()) { return NULL; }
...
void qthread_internal_net_driver_runtime_barrier(void) { PMI_Barrier(); }

int qthread_internal_net_driver_get_rank(void) { return rank; }

int qthread_internal_net_driver_get_size(void) { return size; }
