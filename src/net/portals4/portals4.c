...
static inline int qthread_internal_net_driver_initialize_matching(void) {
  ...
  ret = PtlSetMap(ni_h, qthread_internal_net_driver_get_size(), desired);
  ...
  ret =
    PtlMEAppend(ni_h, data_pt, &me, PTL_OVERFLOW_LIST, NULL, &overflow_me_h);
  if (PTL_OK != ret) {
...
static inline int qthread_internal_net_driver_initialize_no_matching(void) {
  ...
  ret = PtlSetMap(nm_ni_h, qthread_internal_net_driver_get_size(), desired);
  if ((PTL_OK != ret) && (PTL_IGNORED != ret)) {
    fprintf(stderr, "PtlSetMap: %d\n", ret);
    return ret;
  }
  ...
  ret = PtlPTAlloc(nm_ni_h, 0, nm_eq_h, 20, &nm_data_pt);
  if (PTL_OK != ret) {
    fprintf(stderr, "PtlPTAlloc: %d\n", ret);
    return ret;
  }
  ...
  ret = PtlMDBind(nm_ni_h, &md, &nm_md_h);
  if (PTL_OK != ret) {
    fprintf(stderr, "PtlMDBind: %d\n", ret);
    return ret;
  }
...
int qthread_internal_net_driver_initialize(void) {
  ...
  size_recv_block = qt_internal_get_env_num(
    "PORTALS_RECV_BLOCK_SIZE", 1024 * 1024, 1024 * 1024);
...
int qthread_internal_net_driver_register(
  int tag, qthread_internal_net_driver_handler handler) {
  if ((tag <= 0) || (tag >= 256)) { return -1; }
        ...
        case PTL_EVENT_PUT:
          ...
          if (NULL == handlers[ev.hdr_data]) {
            qthread_debug(MULTINODE_CALLS,
                          "Got message with unregistered tag %ld, ignoring\n",
        ...
        case PTL_EVENT_ACK:
          if (PTL_NI_OK != ev.ni_fail_type) {
            ...
            if (ev.mlength != pkt->len) {
              if (PTL_OK != ret) {
                qthread_debug(
                  MULTINODE_CALLS, "transmit packet failed: %d\n", ret);
              }
            } else {
...
static void repost_recv_block(struct recv_block_t *block) {
  ...
  ret = PtlMEAppend(ni_h, data_pt, &me, PTL_PRIORITY_LIST, block, &block->me_h);
  if (PTL_OK != ret) { abort(); }
}
