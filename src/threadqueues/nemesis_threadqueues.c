...
static QINLINE qt_threadqueue_t *ALLOC_THREADQUEUE(
...
static QINLINE void FREE_THREADQUEUE(
    qt_threadqueue_t * t)
{				       /*{{{ */
    qt_mpool_free(t->
...
void INTERNAL qt_threadqueue_init_pools(
    qt_threadqueue_pools_t * p)
{				       /*{{{ */
    assert(p);
    p->nodes = NULL;
...
void INTERNAL qt_threadqueue_destroy_pools(
    qt_threadqueue_pools_t * p)
{				       /*{{{ */
    assert(p);
    assert(p->queues);
...
qt_threadqueue_t INTERNAL *qt_threadqueue_new(
...
void INTERNAL qt_threadqueue_free(
    qt_threadqueue_t * q)
{				       /*{{{ */
    assert(q);
...
void INTERNAL qt_threadqueue_enqueue(
...
void INTERNAL qt_threadqueue_enqueue_yielded(
    qt_threadqueue_t * q,
    qthread_t * t,
    qthread_shepherd_t * shep)
{				       /*{{{ */
...
ssize_t INTERNAL qt_threadqueue_advisory_queuelen(
    qt_threadqueue_t * q)
{				       /*{{{ */
    assert(q);
...
static inline NEMESIS_entry *qt_internal_NEMESIS_dequeue(
    NEMESIS_queue * q)
{				       /*{{{ */
...
qthread_t INTERNAL *qt_threadqueue_dequeue(
    qt_threadqueue_t * q)
{				       /*{{{ */
...
qthread_t INTERNAL *qt_threadqueue_dequeue_blocking(
    qt_threadqueue_t * q)
{				       /*{{{ */
    NEMESIS_entry *retval = qt_internal_NEMESIS_dequeue(&q->q);
