...
void
unmap_from_loc (future_t *ft) {
  DBprintf ("Unmap future %p from loc\n", ft);
  cp_hashtable_remove(qtd_loc, ft);
}

void
map_to_loc (future_t *ft, location_t *loc) {
#if 1
...
void
future_init(qthread_t *qthr, int vp_per_loc, int loc_count) {
  int i;
  init_loc_hash();
...
inline void
blocking_vp_incr (qthread_t *qthr, location_t *loc) {
...
void
future_join(qthread_t *qthr, future_t* ft) {
  DBprintf ("Qthread %p join to future %p\n", qthr, ft);
  qthread_join(qthr, ft);
}

void
future_exit(qthread_t *qthr) {
#if 1
  if (ft_loc(qthr) == NULL) {
