...
class UTSvisit : public task {
	...
	UTSspawnChildren(UTSspawnChildren &x, split) : parent(x.parent), sum(0) {}
	...
	void join(const UTSspawnChildren&y) { sum += y.sum; }
    };
#endif
    public:
    UTSvisit(node_t *p, uint64_t*r) : parent(*p), ret(r) {}
