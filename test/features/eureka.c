...
static void alive_check(size_t const a, size_t const b, void *junk) { return; }

static aligned_t live_waiter(void *arg) {
  int const assigned = (int)(intptr_t)arg;
  int const id = qthread_id();
...
static aligned_t live_parent(void *arg) {
  ...
  qthread_feb_status(
    &alive[0]); // guarantee the other task is not still in the lock
  qthread_feb_status(
    &alive[1]); // guarantee the other task is not still in the lock
...
static aligned_t parent_eureka(void *arg) {
  iprintf(
    "parent_eureka alive! id = %u teamid = %u\n", qthread_id(), qt_team_id());
  qthread_empty(alive + 0);
  qthread_empty(alive + 1);
  qthread_spawn(live_parent_waiter,
                (void *)(intptr_t)0,
                0,
                NULL,
                0,
...
aligned_t return_one(void *arg) { return 1; }
...
int main(int argc, char *argv[]) {
  ...
  snprintf(stack_size,
           20,
           "%u",
           4096 * 2); // XXX: The root cause of this is hard to track down
  ...
  iprintf(
    "\n\n***************************************************************\n");
  iprintf(
    "Testing an over-subscribed eureka (some member tasks not running)...\n");
