...
{
{
    ...
    long p_lower_;
    long p_upper_;
    XOMP_loop_dynamic_init(&xomp_loop_struct_1,0,nseqs - 1,1,1);
    if (XOMP_loop_dynamic_start(xomp_loop_struct_1,0,nseqs - 1,1,1,&p_lower_,&p_upper_)) {
      do {
        for (p_index_ = p_lower_; p_index_ <= p_upper_; p_index_ += 1) {
          if ((_p_n = seqlen_array[p_index_ + 1]) != 0) {
            for (((_p_i = 1) , _p_len1 = 0); _p_i <= _p_n; _p_i++) {
              char c = seq_array[p_index_ + 1][_p_i];
              if (((c) != gap_pos1) && ((c) != gap_pos2)) 
                _p_len1++;
            }
            printf("len %p %d n %d\n",&_p_len1,_p_len1,_p_n);
            for (_p_sj = (p_index_ + 1); _p_sj < nseqs; _p_sj++) {
              if ((_p_m = seqlen_array[_p_sj + 1]) != 0) {
                ...
                __out_argv1__1527__.OUT__1__1527___data::si = p_index_;
      ...
      }while (XOMP_loop_dynamic_next(xomp_loop_struct_1,&p_lower_,&p_upper_));
    }
