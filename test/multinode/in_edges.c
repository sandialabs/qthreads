...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < num_edges; i++) {
    printf(
      "[%03d] edges[%d]: (%lu,%lu)\n", here, i, edges[i].lid, edges[i].vid);
  }
  ...
  for (int i = 0; i < num_edges; i++) { qthread_readFF(&rets[i], &rets[i]); }
