...
static aligned_t ping_comm(void *args_) { return pong_comm(comm_bins); }
...
static aligned_t visit(void *args_) {
  ...
  for (int i = 0; i < num_children; i++) {
    ...
    if (talk_to_self || (target != here)) {
      ...
      qthread_fork_remote(visit, &child, &rets[i], target, sizeof(node_t));
    } else {
...
#else  /* ifdef PRINT_STATS */
  iprintf("Num messages = %lu, Tree size = %lu, tree depth = %d, num leaves = "
          "%llu (%.2f%%)\n",
          (unsigned long)num_messages,
          (unsigned long)total_num_nodes,
          (int)tree_height,
