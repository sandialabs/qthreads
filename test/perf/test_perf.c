...
typedef enum { S1, S2, S3, S4, S5, NumStates1 } state1_t;

char const *state1_names[] = {"S1", "S2", "S3", "S4", "S5"};

typedef enum { S21, S22, S23, S24, S25, NumStates2 } state2_t;

char const *state2_names[] = {"S21", "S22", "S23", "S24", "S25"};

typedef enum { S31, S32, S33, S34, S35, NumStates3 } state3_t;

char const *state3_names[] = {"S31", "S32", "S33", "S34", "S35"};
...
void checked_transition(qtperfdata_t *data, qtperfid_t newstate) {
  ...
  if (last_state != QTPERF_INVALID_STATE) {
    bool valid_timing = data->counters->data[last_state] ==
                        (data->time_entered - entered) + before;
    if (!valid_timing) {
      qtlog(LOGERR, "Invalid timing update");
      qtlogargs(
        LOGERR, "\tstates: %lu -> %lu", last_state, data->current_state);
      qtlogargs(LOGERR, "\tentered: %lu", entered);
      qtlogargs(LOGERR, "\tleft: %lu", data->time_entered);
      qtlogargs(LOGERR, "\tcounter before: %llu", before);
      qtlogargs(
        LOGERR, "\tcounter after: %llu", data->counters->data[last_state]);
      qtlogargs(
        LOGERR, "\texpected difference: %lu", data->time_entered - entered);
      qtlogargs(LOGERR,
                "\tactual difference: %llu",
                data->counters->data[last_state] - before);
    }
...
static size_t const NumThreads = 100;
...
static void test_fake_concurrent(void **state) {
  ...
  for (i = 0; i < NumThreads * 20; i++) {
    checked_transition(threads[i % NumThreads],
                       qtperf_now() % numstates[i % 3]);
    spin(500);
  }
  for (i = 0; i < NumThreads; i++) {
...
static void test_iterator(void **state) {
  ...
  i = 0;
  for (iterdata = qtperf_iter_next(&iter); iterdata != NULL;
...
static void test_print(void **state) {
  ...
  assert_true(group[0] != NULL && group[1] != NULL && group[2] != NULL);
  qtperf_start();
  for (i = 0; i < 5; i++) { threads[i] = qtperf_create_perfdata(group[i % 3]); }
  for (i = 0; i < 5 * 20; i++) {
