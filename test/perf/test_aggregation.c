...
static void test_feblock(void **state) {
  ...
  for (i = 0; i < NTHREAD; i++) {
    qtperfdata_t *spindata =
      qtperf_create_aggregated_perfdata(spingroup, agg_target);
    all_data[i] = spindata;
    if (agg_target == NULL) { agg_target = spindata->counters; }
  ...
  for (i = 0; i < NTHREAD; i++) { qthread_readFE(NULL, &ret); }
