...
// simple.
static inline int spin_lock(uint32_t volatile *busy);

static inline int spin_lock(uint32_t volatile *busy) {
  int ret = 0;
  while (qthread_cas32(busy, 0, 1) != 0) { ret = 1; }
  return ret;
}
...
char const *spin_names[] = {
  "SPIN_WORKING", "SPIN_WAITING", "SPIN_COMPLETE", "SPIN_NUM_STATES"};
...
aligned_t struct_edit(void *data) {
  ...
  for (i = 0; i < 100; i++) {
    ...
    qtperf_enter_state(mydata, SPIN_WORKING);
    start = strct;
    for (j = 0; j < 100000; j++) { strct = strct + 1; }
...
static void test_spinlock(void **state) {
  ...
  qthread_initialize();
  spingroup =
    qtperf_create_state_group(SPIN_NUM_STATES, "Spin Testing", spin_names);
  totalgroup =
...
int main(int argc, char **argv) {
  const struct CMUnitTest test[] = {cmocka_unit_test(test_spinlock)};
