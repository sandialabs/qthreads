...
typedef enum { SS_RUNNING, SS_DONE, SS_NUM_STATES } startstop_t;

char const *names[] = {"SS_RUNNING", "SS_DONE", "SS_NUM_STATES"};
...
void test_startstop(void **state) {
  ...
  for (i = 0; i < num_threads; i++) {
    ...
    if (i % 3 == 0) qtperf_stop();
    if (i % 4 == 0) qtperf_start();
  }
  qtperf_stop();
...
int main(int argc, char **argv) {
  const struct CMUnitTest test[] = {cmocka_unit_test(test_startstop)};
