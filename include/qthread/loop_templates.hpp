...
public:
  static void Run(ObjT *obj,
    ...
    switch (TypeC) {
      case mt_loop_traits::ParNoJoin: join = false;
      case mt_loop_traits::Par: {
        ...
        for (int i = 0; i < tdc; i++) {
          ...
          qthread_fork(
            run_qtd<Iter>, ITER(start, steptd, count), join ? (thr + i) : NULL);
...
#define PAR_LOOP()                                                             \
  ...
  void mt_loop_returns(                                                        \
    const RetV &ret, FptrT fptr, A_LIST int start, int stop, int step = 1) {   \
    void *my_null = NULL;                                                      \
...
#define PAR_VOID_LOOP()                                                        \
  ...
  void mt_loop(FptrT fptr, A_LIST int start, int stop, int step = 1) {         \
    void *my_null = NULL;                                                      \
...
#define PAR_MEMBER_LOOP()                                                      \
  template <class RetB,                                                        \
            ...
            class RetV VC_LIST,                                                \
            class ObjT>                                                        \
  void mt_mfun_loop_returns(ObjT *obj,                                         \
                            const RetV &ret,                                   \
                            FptrT fptr,                                        \
                            A_LIST int start,                                  \
                            int stop,                                          \
...
#define PAR_VOID_MEMBER_LOOP()                                                 \
  ...
  void mt_mfun_loop(                                                           \
    ObjT *obj, FptrT fptr, A_LIST int start, int stop, int step = 1) {         \
    void *my_null = NULL;                                                      \
...
#define A_LIST                                                                 \
  const Arg1V &arg1, const Arg2V &arg2, const Arg3V &arg3, const Arg4V &arg4,  \
    const Arg5V &arg5,
...
void mt_assign_loop(T *array, int size, T init_val) {
  struct func {
    static void apply(T &loc, T init_val) { loc = init_val; }
  };

  mt_loop<ArrayPtr, Val, mt_loop_traits::Par>(
    func::apply, array, init_val, 0, size);
}
