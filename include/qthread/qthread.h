...
#error Qthreads requires either mutex increments, inline assembly, or compiler CAS builtins
  ...
  do {
    ...
    __asm__ __volatile__(
      ...
      : "r"(newval.i), "r"(operand), "0"(oldval.i) /* load into EAX */
      : "cc", "memory");
  ...
  retval.f = *(float volatile *)operand;
  do {
    oldval.f = retval.f;
    newval.f = oldval.f + incr;
...
#define QTHREAD_PIC_REG_4 "b"
  ...
  retval.f = *(double volatile *)operand;
  do {
    oldval.f = retval.f;
    newval.f = oldval.f + incr;
    __asm__ __volatile__("dmb sy\n\t"
...
static QINLINE uint32_t qthread_cas32(uint32_t *operand,
  ...
  __asm__ __volatile__(
    ...
    : "r"(newval), "r"(operand), "0"(oldval) /* load into EAX */
    : "cc", "memory");
  return retval;
...
#define QTHREAD_PIC_REG_4 "b"
  ...
  __asm__ __volatile__(QTHREAD_PIC_PREFIX
                       "lock; cmpxchg8b (%2)" QTHREAD_PIC_SUFFIX
                       : /*EAX*/ "=a"(ret.s.l),
                         /*EDX*/ "=d"(ret.s.h)
                       : "r"(operand),
                         /*EAX*/ "a"(oldv.s.l),
...
static QINLINE aligned_t qthread_cas_xx(aligned_t *addr,
                                        ...
                                        size_t length) { /*{{{ */
  switch (length) {
    case 4: return qthread_cas32((uint32_t *)addr, oldval, newval);

    case 8: return qthread_cas64((uint64_t *)addr, oldval, newval);
