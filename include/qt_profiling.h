...
#define QTHREAD_WAIT_TIMER_STOP(ME, TYPE)                                      \
  do {                                                                         \
    ...
    qtimer_destroy(wait_timer);                                                \
  } while (0)
#define QTHREAD_FEB_TIMER_DECLARATION(TYPE)                                    \
  qtimer_t TYPE##_timer = qtimer_create();
...
#define QTHREAD_FEB_TIMER_STOP(TYPE, ME)                                       \
  do {                                                                         \
    ...
    (ME)->rdata->shepherd_ptr->TYPE##_count++;                                 \
    qtimer_destroy(TYPE##_timer);                                              \
  } while (0)
#define QTHREAD_HOLD_TIMER_INIT(LOCKSTRUCT_P)                                  \
  (LOCKSTRUCT_P)->hold_timer = qtimer_create()
#define QTHREAD_HOLD_TIMER_START(LOCKSTRUCT_P)                                 \
  qtimer_start((LOCKSTRUCT_P)->hold_timer)
#define QTHREAD_HOLD_TIMER_STOP(LOCKSTRUCT_P, SHEP)                            \
  do {                                                                         \
    ...
    if ((SHEP)->hold_maxtime < secs) { (SHEP)->hold_maxtime = secs; }          \
    (SHEP)->hold_time += secs;                                                 \
  } while (0)
#define QTHREAD_HOLD_TIMER_DESTROY(LOCKSTRUCT_P)                               \
  qtimer_destroy((LOCKSTRUCT_P)->hold_timer)
#define QTHREAD_EMPTY_TIMER_INIT(LOCKSTRUCT_P)                                 \
  (LOCKSTRUCT_P)->empty_timer = qtimer_create()
#define QTHREAD_EMPTY_TIMER_START(LOCKSTRUCT_P)                                \
  qtimer_start((LOCKSTRUCT_P)->empty_timer)
#define QTHREAD_EMPTY_TIMER_STOP(LOCKSTRUCT_P)                                 \
  do {                                                                         \
    ...
    if (ret->empty_maxtime < secs) { ret->empty_maxtime = secs; }              \
    ret->empty_time += secs;                                                   \
    ret->empty_count++;                                                        \
  } while (0)
#define QTHREAD_FEB_UNIQUERECORD(TYPE, ADDR, ME)                               \
  qt_hash_put((ME)->rdata->shepherd_ptr->unique##TYPE##addrs,                  \
              (void *)(ADDR),                                                  \
              (void *)(ADDR))
...
#define QTHREAD_WAIT_TIMER_START()                                             \
  do {                                                                         \
  } while (0)
#define QTHREAD_WAIT_TIMER_STOP(ME, TYPE)                                      \
  do {                                                                         \
...
#define QTHREAD_FEB_TIMER_DECLARATION(TYPE)
#define QTHREAD_FEB_TIMER_START(TYPE)                                          \
  do {                                                                         \
  } while (0)
#define QTHREAD_FEB_TIMER_STOP(TYPE, ME)                                       \
  do {                                                                         \
